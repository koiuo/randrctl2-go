...
PACKAGE="github.com/edio/randrctl2"
...
${RELEASE}/%-${VERSION}.tar.gz: | release-dir
	...
	go build -ldflags="-s -w -X ${PACKAGE}/cmd.version=${VERSION} -v" -o ${RELEASE}/${EXECUTABLE}
...
${BINARY}: bin-dir
	go build -ldflags="-s -w -X ${PACKAGE}/cmd.version=${VERSION} -v" -o $@
